    ...
    <div class="order-content" v-loading="loading">
      <div class="left item">
        ...
        <div class="content-nav">
          ...
          ><span>{{ data.num }} 张</span> <span>已核销：</span><span>{{ data.useNum }} 张</span> <span>订单联系人：</span><span> {{ data.nickName }}-{{ data.mobile }}</span> <span>使用日期：</span
          ><span v-if="data.validDays > 0">购买之日起 {{ data.visitDate }} 天有效</span>
          <span v-else>{{ data.effectDate }} ~ {{ data.expireDate }}</span>
          <span>使用时间：</span><span> {{ data.effectTime }} ~ {{ data.expireTime }}</span> <span>下单时间：</span><span>{{ data.createTime }}</span>
      ...
      <div class="middle item">
        ...
        <div class="content-nav">
          <span>餐饮券名称：</span><span>{{ data.title }}</span> <span>餐饮店铺：</span><span>{{ data.restaurantName }}</span> <span>备注信息：</span
          ><span
